...
export var renderTemplFull = (files: R2Object[], folders: string[], path: string, config: SiteConfig) => {
  ...
  <html>
    ...
    <body>
      ${svgs}
      <header>
        <h1><a href="/">${config.name}</a> / ${renderTemplBreadcrumbs(path)}</h1>
      </header>
      <main>
        <div class="listing">
      ...
      <footer>${generateFooter(config, path)}</footer>
    </body>
  </html>`;
};
...
var renderTemplFolders = (folders: string[], siteConfig: SiteConfig) => {
  ...
  for (var i = 0; i < folders.length; i++) {
    output += `<tr class="file ">
                  ...
                  <td class="name"><a href="/${folders[i]}">
                    <svg width="1.5em" height="1em" version="1.1" viewBox="0 0 317 259"><use xlink:href="#folder"></use></svg>
                    <span class="name">${cleanFolderName(folders[i])}</span></a>
                  </td>
                  <td class="description">${findDesp(siteConfig, '/' + folders[i].slice(0, -1), true) ?? '-'}</td>
                  <td class="size">-</td>
...
var renderTemplFiles = (files: R2Object[], siteConfig: SiteConfig) => {
  ...
  for (var i = 0; i < files.length; i++) {
    output += `<tr class="file ">
                  <td class="hideable"></td>
                  <td class="name"><a href="/${files[i].key}">
                    <svg width="1.5em" height="1em" version="1.1" viewBox="0 0 265 323"><use xlink:href="#file"></use></svg>
                    <span class="name">${cleanFileName(files[i].key)}</span></a>
                  </td>
                  <td class="description">${findDesp(siteConfig, '/' + files[i].key, true) ?? '-'}</td>
                  <td class="size">${humanFileSize(files[i].size)}</td>
                  <td class="date hideable">
                    <time datetime="${files[i].uploaded.toUTCString()}">${files[i].uploaded.toJSON()}</time>
                  </td>
                  <td class="hideable"></td>
              </tr>`;
  }
